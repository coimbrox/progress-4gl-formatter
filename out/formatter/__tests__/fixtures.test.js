...
for (const [name, src] of Object.entries({
    ...
    (0, node_test_1.test)(`${name}: formats without throwing and is idempotent`, () => {
...
(0, node_test_1.test)('validaImposto: where/and operators land in the same column', () => {
    const lines = (0, index_1.formatSource)(VALIDA_IMPOSTO_FUNCTION).split('\n');
    const eqCols = lines
...
(0, node_test_1.test)('def var block: name column is aligned within each blank-line-separated group', () => {
    const lines = (0, index_1.formatSource)(DEF_VAR_BLOCK).split('\n').filter(l => l.trim().length > 0);
    const likeOrAsCol = (l) => {
...
(0, node_test_1.test)('temp-table block: field names are aligned before like/as', () => {
    const lines = (0, index_1.formatSource)(TEMP_TABLE_BLOCK).split('\n').filter(l => l.includes('field'));
    const likeCol = lines.map(l => l.indexOf('like'));
    assert.ok(likeCol.every(c => c === likeCol[0]));
});
(0, node_test_1.test)('for each ... where block opens a block (colon) and its body is indented', () => {
    const lines = (0, index_1.formatSource)(FOR_EACH_WHERE_BLOCK).split('\n');
    const messageLine = lines.find(l => l.includes('message'));
    assert.ok(messageLine.startsWith('  '));
});
(0, node_test_1.test)('if/and chain: comparison operators align across if and and', () => {
    const lines = (0, index_1.formatSource)(IF_AND_CHAIN).split('\n');
