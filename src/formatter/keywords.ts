 ...
 */
export const KEYWORD_CASING: Record<string, string> = {
    'define': 'def', 'def': 'def',
...
export function isKeyword(text: string): boolean {
    return Object.prototype.hasOwnProperty.call(KEYWORD_CASING, text.toLowerCase());
}

export function canonicalKeyword(text: string): string {
    return KEYWORD_CASING[text.toLowerCase()] ?? text.toLowerCase();
}
